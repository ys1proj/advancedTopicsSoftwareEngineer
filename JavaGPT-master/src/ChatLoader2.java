...
public class ChatLoader2 extends JFrame {

	
	private final String api_key = "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";
	private final String model_name = "fine-tuned-gpt2";
	...
	/*public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ChatLoader2 frame = new ChatLoader2();
    ...
    public static void loadConfig() {
        try {
            ...
            // Retrieve API key and model from the properties file
            String apiKey = prop.getProperty("apikey");
            String model = prop.getProperty("model");

            // Check if API key and model name are correctly loaded
            if (apiKey == null || model == null) {
                throw new IllegalArgumentException("API key or model name not found in configuration.");
            }

            // Assign loaded properties to the MainFrame variables
//            MainFrame.YOUR_API_KEY = apiKey;
//            MainFrame.YOUR_MODEL_NAME = model;
	 ...
	 * Create the frame.
	 */
	public ChatLoader2(String path) {
		this.path = path;
